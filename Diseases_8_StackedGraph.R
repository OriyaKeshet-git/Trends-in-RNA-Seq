...
types = c("cancer OR prostate cancer OR breast cancer OR lymphoma OR leukemic OR leukemia OR osteosarcoma OR breast cancer OR prostate cancers OR ovarian cancer OR colorectal carcinoma OR lung cancer OR 
          ...
          TSS OR aureus infection OR viral OR malaria OR malaria parasite OR HCV OR HCV infection OR metabolic impact of HCV infection OR EBV OR Dengue viruses OR Dengue Fever Vector OR viral infections OR viral infection OR virus infection 
          OR parasite OR parasitic OR parasitic diseases OR fungal meningitis OR meningitis OR fungal infection OR fungal [Title/Abstract]",
          
          "liver OR liver disease OR liver cirrhosis OR liver toxicity OR kidney OR chronic kidney disease OR gastro OR gastric OR gastrointestinal disease OR gastrointestinal OR inflammatory bowel disease OR metabolic diseases OR metabolic syndrome OR 
          hepatic metabolism and liver disease OR metabolic disorder OR metabolic disease OR colonic polyp lesion [Title/Abstract]",
          
          "blood OR atherosclerosis OR MDS OR Myelodysplastic OR myelodysplastic syndrome OR cotton SE OR SE OR diabetes OR diabetes-induced memory deficits [Title/Abstract]",
          
          "heart OR cardiac OR cardio OR cardiac hypertrophy OR cardiac septation defects OR myocardial ischemia OR cardiovascular diseases OR myocardial infarction OR cardiac dysfunction OR Congenital heart disease OR PHD OR
...
pdf("Diseases Stacked Graph (Merged).pdf", width = 15, height = 10)

theme_set(theme_classic())

  ggplot(df, aes(x=Year, y=Value, fill=Diseases)) + 
  geom_area(alpha=0.9 , size=0.01, colour="grey", aes(fill=forcats::fct_reorder(Diseases, Value)), stat="identity") +
